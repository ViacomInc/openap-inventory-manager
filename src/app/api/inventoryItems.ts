...
// this is for CSV
export const createInventoryItemsRequest = (items: InventoryItemInput[]) => (
  dispatch: Dispatch,
  _getState: () => State
): void => {
  graphqlRequest<
...
export const selectRemoveInventoryItemRequest = (id: number) => (
  state: State
): RequestWithData<InventoryItem> => ({
...
export const selectRestoreInventoryItemRequest = (id: number) => (
  state: State
): RequestWithData<InventoryItem> => ({
...
export const updateInventoryItemRequest = (update?: InventoryItemUpdate) => (
  dispatch: Dispatch,
  getState: () => State
): void => {
  const state = getState();
...
export const selectUpdateInventoryItemRequest = (id: number) => (
  state: State
): RequestWithData<InventoryItem> => ({
  ...state.requests[makeUpdateKey(id)],
  data: state.inventoryItems[id],
...
export const selectInventoryItemTransation = (id: number) => ({
  requests,
  transaction,
}: State): SelectInventoryItemTransation => {
...
export const submitInventoryItemsRequest = (publisherId: number) => (
  dispatch: Dispatch
): void => {
  graphqlRequest<
    SubmitInventoryItemsMutationResult,
...
export const selectSubmitInventoryItemsRequest = () => (
  state: State
): Request => state.requests[SubmitInventoryItemsRequestKey] || {};

export const flushInventoryItemsRequest = ({ start, end }: Interval) => (
  dispatch: Dispatch
): void => {
  graphqlRequest<
    FlushInventoryItemsMutationResult,
...
export const selectFlushInventoryItemsRequest = () => (state: State): Request =>
